...
Write-Host "Starting Docker and Postgres installation..." -ForegroundColor Green
...
$containerName = PromptUser -PromptText "Enter the container name" -DefaultValue "postgres"
$networkName = PromptUser -PromptText "Enter the network name" -DefaultValue "general"
$postgresPort = PromptUser -PromptText "Enter the Postgres port" -DefaultValue "5432"
$postgresDatabase = PromptUser -PromptText "Enter the Postgres database name" -DefaultValue "general"
$postgresUser = PromptUser -PromptText "Enter the Postgres user" -DefaultValue "admin"
$postgresPassword = PromptUser -PromptText "Enter the Postgres password" -DefaultValue "adminpassword"
$allowHost = PromptUser -PromptText "Enter the allowed host (Default: 0.0.0.0)" -DefaultValue "0.0.0.0"

# Generate the .env file
Write-Host "Creating .env file for Postgres setup..." -ForegroundColor Green
$envContent = @"
...
if (Test-Path "docker-compose.example.yaml") {
    $exampleContent = Get-Content "docker-compose.example.yaml" | ForEach-Object {
        $_ -replace "\$\{CONTAINER_NAME\}", $containerName `
            ...
            -replace "\$\{POSTGRES_DATABASE\}", $postgresDatabase `
...
Write-Host "Starting Docker Compose with --build for Postgres..." -ForegroundColor Green
try {
    docker-compose up -d --build
    $containerStatus = docker inspect -f '{{.State.Running}}' $containerName
    if ($containerStatus -eq "true") {
        Write-Host "Postgres setup is complete and running. Connecting to the container..." -ForegroundColor Green
